...
def setup_environment(seed=2024):
...
def emission_probability(state, observation,sigma):
    ...
    stateCoords = np.array(state[0])
    obsCoords = np.array(observation)

    diffVector = obsCoords - stateCoords
    mahalanobisDist = np.dot(diffVector, diffVector)
    normalizationC = 2 * np.pi * sigma**2
    return -0.5 * (mahalanobisDist / (sigma**2) + np.log(normalizationC))

valid_moves_cache = {}
def transition_probability(prev_state, curr_state, movementPolicy):
    """
    ...
    prevPos, prev_heading = prev_state
    currPos, curr_heading = curr_state

    # Use cache if available
    if prevPos not in valid_moves_cache:
        validMoves = [(prevPos[0] + dx, prevPos[1] + dy) for dx, dy in MOVEMENTS.values() 
                       if not is_obstacle((prevPos[0] + dx, prevPos[1] + dy))]   
        valid_moves_cache[prevPos] = validMoves
    else:
        validMoves = valid_moves_cache[prevPos]

    if movementPolicy == "random_walk":
        if currPos in validMoves:
            return -np.log(len(validMoves))
        return -np.inf
    
    elif movementPolicy == "straight_until_obstacle":
        dx, dy = MOVEMENTS[prev_heading]
        predictedPos = (prevPos[0] + dx, prevPos[1] + dy)
        if currPos == predictedPos:
            return 0.0
        elif currPos != prevPos:
            return -np.log(len(MOVEMENTS))
        return -np.inf
...
def viterbi(observations, start_state, movement_policy,states,sigma):
    ...
    numSteps = len(observations)
    probabilityTable = [{}]
    statePaths = {}

    for state in states:
        transitionScore = transition_probability(start_state, state, movement_policy)
        observationScore = emission_probability(state, observations[0], sigma)
        if state == start_state:
            probabilityTable[0][state] = transitionScore + observationScore
        else:
            probabilityTable[0][state] = -np.log(len(states))
        statePaths[state] = [state]
    ...
    for t in range(1, numSteps):
        ...
        for currentState in states:
            ...
            for previousState in states:
                if previousState in probabilityTable[t - 1]:
                    prevProb = probabilityTable[t - 1][previousState]
                    transitionScore = transition_probability(previousState, currentState, movement_policy)
                    observationScore = emission_probability(currentState, observations[t], sigma)
                    totalScore = prevProb + transitionScore + observationScore
                    if totalScore > maxProb:
                        maxProb = totalScore
                        bestPrevious = previousState
            probabilityTable[t][currentState] = maxProb
            newPaths[currentState] = statePaths[bestPrevious] + [currentState]
        statePaths = newPaths
    
    finalState = max(probabilityTable[-1], key=probabilityTable[-1].get)
    return statePaths[finalState]
...
if __name__ == "__main__":
    ...
    import csv
    # Save estimated paths to a CSV file
    with open("estimated_paths.csv", mode="w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["Seed", "Policy", "Estimated Path"])
        for seed, policy_data in results.items():
            for policy in policies:
                writer.writerow([seed, policy, getestimatedPath(policy, results, states, sigma)[1]])